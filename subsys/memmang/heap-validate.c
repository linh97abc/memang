...
void heap_print_info(struct z_heap *h, bool dump_chunks)
{
	...
	__ASSERT_PRINT("Heap at %p contains %d units in %d buckets\n\n",
	       chunk_buf(h), h->end_chunk, nb_buckets);

	__ASSERT_PRINT("  bucket#    min units        total      largest      largest\n"
	...
	for (i = 0; i < nb_buckets; i++) {
		...
		if (count) {
			__ASSERT_PRINT("%9d %12d %12d %12d %12zd\n",
	...
	if (dump_chunks) {
		...
		for (chunkid_t c = 0; ; c = right_chunk(h, c)) {
			__ASSERT_PRINT("chunk %4d: [%c] size=%-4d left=%-4d right=%d\n",
			       c,
			       chunk_used(h, c) ? '*'
